...
export default function CabExplorer() {
  ...
  // ── Load recent cabinets and current cabinet on mount ──────────────────────
  ...
  async function handleOpenFile() {
    setError(null)
    const dialogResult = await window.dialog.openFile()
    if (!dialogResult.ok || !dialogResult.data) return

    const result = await window.cabinet.open(dialogResult.data)
  ...
  async function handleOpenRecent(path: string) {
    setError(null)
    const result = await window.cabinet.open(path)
    if (!result.ok) {
      setError(result.error)
      // Remove from recent if file not found
      if (result.error.includes('not found')) {
        await window.recent.remove(path)
        await refresh()
      }
      return
    }
  ...
  async function handleClone(sourcePath: string, sourceName: string) {
    setError(null)
    const dialogResult = await window.dialog.saveFile(`${sourceName}-copy.cabinet`)
    if (!dialogResult.ok || !dialogResult.data) return

    const result = await window.cabinet.clone({
      sourcePath,
      destPath: dialogResult.data,
    })
    if (!result.ok) { setError(result.error); return }
    await refresh()
  }
  ...
  async function handleDelete(path: string, name: string) {
    ...
    const result = await window.cabinet.delete(path)
    if (!result.ok) { setError(result.error); return }
  ...
  // ── Remove from recent (not delete file) ──────────────────────────────────
  ...
  return (
    <div
      ...
      style={{ height: '100%', backgroundColor: 'var(--background)', position: 'relative' }}
    >
      {/* Toolbar */}
      <div
      ...
      >
        ...
        {error && (
          <span style={{ fontSize: 11, color: '#BD2A49', flex: 1, textAlign: 'center' }}>
            {error}
          </span>
        )}
      ...
      {newCabModal && (
        <NewCabinetModal
          ...
          onCreate={async (path, name) => {
            setError(null)
            const result = await window.cabinet.create({ path, name })
            if (!result.ok) { setError(result.error); return }
            setActivePath(result.data.path)
    ...
    <div className="flex flex-1 flex-col items-center justify-center gap-4">
      <div style={{ color: 'var(--muted-foreground)' }}>
        <svg viewBox="0 0 48 48" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ width: 48, height: 48 }}>
          ...
          <path d="M6 16h36" />
          <path d="M16 28h16M16 33h10" strokeLinecap="round" />
        </svg>
      </div>
...
function CabinetCard({
  cabinet, isHovered, hoveredAction,
  onMouseEnter, onMouseLeave, onActionEnter, onActionLeave,
  onClick, onClone, onDelete, onRemove,
}: {
    ...
    >
      ...
      <div className="flex items-center gap-3" style={{ marginBottom: isHovered ? 12 : 0 }}>
        <span className="ml-auto" style={{ fontSize: 11, color: 'var(--muted-foreground)' }}>
          {formatRelativeTime(cabinet.lastOpenedAt)}
        </span>
      </div>
          ...
          <div className="flex items-center gap-1">
            {[
              ...
              <ActionButton
                ...
                destructive={false}
                onMouseEnter={() => onActionEnter(label)}
            ...
            <ActionButton
              label="Delete"
              isHovered={hoveredAction === 'Delete'}
              destructive
              onMouseEnter={() => onActionEnter('Delete')}
              onMouseLeave={onActionLeave}
              onClick={onDelete}
            />
          </div>
...
function NewCabinetModal({
  ...
  async function pickPath() {
    const defaultName = name.trim()
      ...
      : 'new-cabinet.cabinet'
    const result = await window.dialog.saveFile(defaultName)
    ...
    <>
      ...
      >
        ...
        >
          ...
          <button
            onClick={onClose}
            style={{ marginLeft: 'auto', color: 'var(--muted-foreground)', background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}
          >
        ...
        <div className="flex flex-col gap-4" style={{ padding: '20px 20px 8px' }}>
          <div className="flex flex-col gap-1.5">
            ...
            <input
              type="text"
              placeholder="e.g. Film Collection"
              value={name}
            ...
            <p style={{ fontSize: 10, color: 'var(--muted-foreground)' }}>
              If left blank, the name is derived from the filename.
            </p>
          </div>
        ...
        <div
          ...
          style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', backgroundColor: 'var(--secondary)', borderRadius: '0 0 var(--radius) var(--radius)' }}
        >
          ...
          <button
            ...
            style={{
              ...primaryBtnStyle,
              opacity: !path || saving ? 0.5 : 1,
              cursor: !path || saving ? 'not-allowed' : 'pointer',
            }}
          >
